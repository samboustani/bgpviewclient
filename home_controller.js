...
app.controller('home', ['$scope', 'apiServices', '$q', function($scope, apiServices, $q) {
    ...
    $scope.getPrefix = function(prefix) {
        getPrefixDetails(prefix)
            .then(function(data) {
                ...
                getWhoIs(prefix, 'prefix')
                .then(function(data) {
                    $scope.prefixdata.whois = data;
                });
    ...
    function getAsnDetails(asn) {
        ...
        apiServices.getAsnDetails(asn)
            .then(function (response) {
                ...
                $scope.asndata.asn_ipv4addresses = $scope.searchdata.ipv4count == 200 ? '200+' : $scope.searchdata.ipv4count;
                d.resolve(true);
            });
            return d.promise;
    }
    ...
    function getWhoIs(queryText, queryType) {
        var d = $q.defer();
        apiServices.getWhoIs(queryText, queryType)
            .then(function (response) {
                d.resolve(JSON.stringify(response.data, null, 4));
            });
            return d.promise;
    }
